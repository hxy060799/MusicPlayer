...
const pinyinTableCell  pinyinTable[] = {
    ...
    {@"Di",@"地第底低敌抵滴帝递嫡弟缔堤涤提笛迪狄翟蒂觌邸谛诋嘀柢骶羝氐棣睇娣荻碲镝坻籴砥仾俤偙僀儥勺厎呧唙啇啲啻嚁坔坘埅埊埞墆墑墬奃媂嵽嶳廸弔弚弤彽怟慸扚拞掋揥摕敵旳杓杕枤梊梑楴樀浟渧滌焍牴玓珶甋疐眱碮磾祶禘篴糴約締约聜肑胝腣芍苐苖莜菂菧蓧蔋蔐蔕藋藡蚳蝃螮袛覿觝詆諟諦豴赿趆踧踶蹄蹏蹢軧逐逓逮遞適遰釱鉪鍉鏑阺隄隶靮鞮頔題题馰髢鬄魡鯳鸐"},
    {@"Dia",@"嗲"},
    ...
    {@"Nin",@"您恁囜拰脌"},
    {@"Ning",@"拧凝宁柠狞泞佞甯咛聍侫儜冰嚀嬣寍寕寗寜寧擰攘橣檸澝濘獰疑矃聹苧薴鑏鬡鬤鸋"},
    ...
    {@"Qing",@"请轻清青情晴氢倾庆擎顷亲卿氰圊謦檠箐苘蜻黥罄鲭磬綮倩傾儬凊剠勍啨埥声夝媇寈庼廎慶掅擏暒棾樈檾櫦殑殸氫涇淸渹漀濪玪甠硘硜碃綪胜莔葝請軽輕郬鑋靑靘頃鯖鶄"},
    ...
    for (int j = 0; j < [hzString length]; j++)
    {
        ...
        if((0x10 <= bh) && (bh <= 0x57))//是gb2312汉字
        {
            ...
            for (int j = 0; j < pyCount; j++)
            {
                ...
                {
                    NSString *result=pinyinTable[j].pinyin;
                    //if([[DFPinYin getNSString:result atIndex:1] isEqualToString:firstLetter])
                    //{
                        pyString = result;
                        isFind = true;
                        //[resultArray addObject:[[NSString stringWithFormat:@"%@",pinyinTable[j].pinyin]uppercaseString]];
                        break;
                    //}
                }
